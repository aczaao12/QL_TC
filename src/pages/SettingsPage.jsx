...
import { addBudget, getAllBudgets, updateBudget, deleteBudget, getBudgetByCategory, syncData, getAiConfig, saveAiConfig } from '../services/db'; // Adjusted path and type-only import
...
export default function SettingsPage() {
  ...
  const [syncEnabled, setSyncEnabled] = useState(false); // This state is for the toggle, not the actual sync status
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="mb-8 p-4 border rounded-md shadow-sm bg-white">
        ...
        {loading ? (
          ...
          <p>Bạn chưa đăng nhập. <Link to="/login" className="text-blue-500 hover:underline">Đăng nhập ngay</Link> để đồng bộ hóa dữ liệu.</p>
        )}
      </div>
      ...
      <div className="mt-8 p-4 border rounded-md shadow-sm bg-white">
        <h2 className="text-xl font-semibold mb-3">Cấu hình AI</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="aiTemp" className="block text-sm font-medium text-gray-700">Nhiệt độ (Temperature)</label>
            <input
              type="number"
              id="aiTemp"
              ...
              onChange={(e) => setAiConfig({ ...aiConfig, temperature: parseFloat(e.target.value) || 0 })}
              step="0.1"
              min="0"
              max="1"
            />
          ...
          <div>
            <label htmlFor="aiTopK" className="block text-sm font-medium text-gray-700">Top K</label>
            <input
              type="number"
              id="aiTopK"
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              value={aiConfig.topK}
              onChange={(e) => setAiConfig({ ...aiConfig, topK: parseInt(e.target.value) || 0 })}
              min="1"
            />
          </div>
          <div>
            <label htmlFor="aiTopP" className="block text-sm font-medium text-gray-700">Top P</label>
            <input
              type="number"
              id="aiTopP"
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              value={aiConfig.topP}
              onChange={(e) => setAiConfig({ ...aiConfig, topP: parseFloat(e.target.value) || 0 })}
              step="0.1"
              min="0"
              max="1"
            />
          ...
          <div>
            <label htmlFor="aiMaxOutputTokens" className="block text-sm font-medium text-gray-700">Max Output Tokens</label>
            <input
              type="number"
              id="aiMaxOutputTokens"
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              value={aiConfig.maxOutputTokens}
              onChange={(e) => setAiConfig({ ...aiConfig, maxOutputTokens: parseInt(e.target.value) || 0 })}
              min="1"
            />
          </div>
        </div>
        <button
          className="mt-6 bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
